...
//2.	ќписать процедуру Smooth(A, N), замен€ющую каждый элемент вещественного массива A размера N на его среднее арифметическое со своими сосед€ми
//("сглаживание массива").ћассив A Ч входной и выходной параметр, N Ч входной параметр.— помощью этой процедуры выполнить п€тикратное сглаживание данного массива 
//A размера N, вывод€ на экран результаты каждого сглаживани€.

//¬вести с клавиатуры массив, состо€щий из n строк и m столбцов.—оздать библиотеку, 
//в которой будет содержатьс€ функци€, котора€ производит вычислени€ согласно заданию, приведенному в таблице.
...
void Smooth(int *a, int r, const int *k)
{
	...
	for (int l = 0; l < *k; l++)
	{
		a++;//увеличиваем кол-во битб только если есть указатель на массив *а
		for (j = 1; j < r - 1;j++)
		{
...
int sum_ind(int(*a)[4], int r,int &col, int &row, int &max)//возвращает более 1 значени€
{
	int s;
	max = *(*a);
	for (int i = 0; i < r; i++)
